...
int fig[10] = {0,9,99,999,9999,99999,99999,999999};
...
if(cnt % 2 ==0){
    ll tmp = n / pow(10,cnt /2 );
        ans = fig[(cnt) /2 -1 ] + tmp +1 - pow(10 ,cnt / 2 -1 );
    if(n - (tmp*pow(10,cnt /2)) < tmp)
        ans = fig[(cnt) /2 -1 ] + tmp - pow(10 ,cnt / 2 -1 ) ; 
        // tmp - (tmp*pow(10,cnt /2));
}
else{
    ans = fig[(cnt -1) /2  ];
}
