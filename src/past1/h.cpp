...
int min_kisu = INFINT, min_all = INFINT;

REP(i,n){
    if(i %2 == 0) //奇数  (indexが１個づれているので)
        min_kisu = min(min_kisu, c[i]);
...
REP(i,q){
    ...
    if(z == 1){
        ...
        x--;
        int card_tmp;
        if(x % 2 == 0) { //奇数 (indexが１個づれているので)
            card_tmp =  c[x] - (sum_kisu  + sum );
        }
        else // 偶数
            card_tmp =  c[x] - sum ;
        if(card_tmp >= a){
            ...
            ans += a;
            if(x % 2 == 0) //奇数なら
                min_kisu = min(min_kisu,c[x] );
            min_all = min(min_all,c[x] );
        }
    }
    if(z ==2){
        cin >> a;
        if(min_kisu - sum_kisu -sum >= a){
            sum_kisu += a;
        }
    }
    if(z ==3){
        cin >> a;
        if(min(min_all - sum , min_kisu - sum - sum_kisu) >= a){
            sum += a;
        }
    }
    continue;
}
ans += sum_kisu * ( (n +1)/ 2) + sum *n;
OUT(ans)
