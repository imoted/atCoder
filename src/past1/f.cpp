...
V<pair<string,string>> mojiretsu;
...
FOR(i,1,s.size()){
    for(auto w : C){
        if(s[i] == w){
            pair<string,string> moji;
            moji.first = s.substr(first_i, i - first_i +1);
            moji.second = s.substr(first_i, i - first_i +1);
            transform(moji.first.begin(), moji.first.end(), moji.first.begin(), ::tolower);
            mojiretsu.push_back(moji);
            i++;
            first_i = i;
            break;
        }
