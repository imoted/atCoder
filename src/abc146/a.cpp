...
string s;
cin >> s;
vector<string> week_list{"SUN","MON","TUE","WED","THU","FRI","SAT"};

int cnt = 0;
for(auto week : week_list){
    if(week == s){
        cout << 7 - cnt << endl;
        break;
    }
...
//     string s;
//     std::cin >> s;
//     std::vector<string> week_name_list{"SUN", "MON"};

//     for(int i =0; i< week_name_list.size(); i++){
//         if(week_name_list[i] == s){
