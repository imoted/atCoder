...
// int dfs(ll x, int n, V<int> c ,ll c_sum,V<ll> a_sum, int sel, int bit){
// 	int cnt=0;
// 	if( x >= c_sum) return c_sum;
// 	if (bit == pow(2,n)) return INFINT;
// 	int c_sum1 = dfs(x, n, c, c_sum, a_sum ,sel,bit++ );
// 	c_sum += c[ 1 << bit ];
// 	// sel = sel | (1 << bit);
// 	int c_sum2 = dfs(x, n, c, c_sum, a_sum ,sel,bit++ );
// 	return min(c_sum1,c_sum2);
// }
