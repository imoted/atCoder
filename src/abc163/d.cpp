...
int a,b;
FOR(i,k,n+2){   // k 〜 n+1までで、回す
	a = i * (2 * (n+1 - i) + i -1) /2;  // 等差数列の和　初項 n+1 -i コウサ１
	b = (i * (i -1) /2 );               // 等差数列の和　初項 0  コウサ１
	ans += a - (b -1);                  // 一番大きな数から、一番小さな数まですべて作れるので、
	// 一番大きな等差数列の和から、一番小さな等差数列を引くことで、Nの個数に従って、ansに足していく
}
