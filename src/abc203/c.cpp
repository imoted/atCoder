...
REP(i, n)
{
    cin >> apair[i].first >> apair[i].second;
}
sort(ALL(apair));
...
REP(i,n){
    if(money - (apair[i].first - last_place) >= 0){
        money += apair[i].second - (apair[i].first - last_place);
        last_place = apair[i].first;
    }
    else{
        break;
    }
}
goal = money + last_place;
OUT(goal) BR
