...
// ll max_x  = 2 * floor(r +1);
ll ans =0;

for(int x = round( (a_int - r_int) /10000) * 10000 - 20000  ; x < round((a_int + r_int ) /10000) *10000 + 20000; x = x + 10000){
    if(pow(r_int,2) - pow(x-a_int ,2) >= 0){
        ld y1 = b_int + sqrt( pow(r_int,2) - pow(x-a_int ,2));
        ld y2 = b_int - sqrt( pow(r_int,2) - pow(x-a_int ,2));
        // if(y1 == y2) ans += 1;
        // else{
            ll y1r = floor(y1 /10000);
            ll y2r = ceil(y2 /10000);
            ans += (y1r - y2r +1);
        // }
    }
}
